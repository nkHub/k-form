<template>
  <div :style="{ cursor: cursor }" :title="title" @mouseleave="onMouseleave" @click="onEvents"
    @contextmenu="onContextmenu" @dblclick="onEvents" @touchstart="onTouchStart" @touchmove="onTouchMove"
    @touchend="onTouchEnd" ref="board">
    <slot />
    <!-- 纵向滚动条 -->
    <div :style="{
      height: bodyHeight + 'px',
    }" v-show="maxScrollHeight > 0" class="k-table-scrolly-box">
      <div ref="scrollY" @click.stop @mousedown.stop="yMousedown" class="k-table-scrolly-bar" :class="{ moving: yIsDown }"
        :style="{ height: scrollyBarHeight + 'px' }"></div>
    </div>
    <!-- 横向滚动条 -->
    <div class="k-table-scrollx-box" v-show="maxScrollWidth > 0">
      <div ref="scrollX" @click.stop @mousedown.stop="xMousedown" :style="{ width: scrollxBarWidth + 'px' }"
        :class="{ moving: xIsDown }" class="k-table-scrollx-bar"></div>
    </div>
    <div @mousedown.stop="colMousedown" @click.stop v-show="currentResizeCol && mouseInfo" :style="resizeColStyle"
      class="k-table-col-resize-handler"></div>
    <div v-show="colIsDown" :style="{ left: parseInt(resizeColStyle.left) + colResizeHandlerWidth + 'px' }"
      class="k-table-col-resize-line"></div>
    <!-- 鼠标移入的气泡 -->
    <tooltip :placement="tooltipPlacement" :getBoundingClientRect="getTooltipRect" :show.sync="showTooltip"
      :content="tooltipContent" />
    <!-- 通用气泡 -->
    <tooltip customStyle="pointer-events: initial;" :placement="popoverPlacement" :getBoundingClientRect="getPopoverRect"
      :show.sync="showPopover">
      <keep-alive>
        <component v-model="queryValue[popoverInfo.queryField]" @cancel="showPopover = false" v-if="popoverInfo.component"
          :is="popoverInfo.component" @confirm="onPopoverConfirm" :show="showPopover" :field="popoverInfo.queryField"
          v-on="popoverInfo.events" v-bind="popoverInfo.attrs"></component>
      </keep-alive>
    </tooltip>
    <input :style="selectInputStyle" type="text" @contextmenu.stop @blur="showSelectInput = false"
      v-show="showSelectInput" ref="selectInput" readonly class="k-table-select-input">
  </div>
</template>
